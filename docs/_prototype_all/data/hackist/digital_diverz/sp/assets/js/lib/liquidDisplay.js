...
(function(){
	...
	function DigitalDiverZ()
	{
		...
		function initObject()
		{
			...
			function changeRipple()
			{
				setTimeout( function(){
					...
					var _mode = Math.floor( Math.random() * _images.length );
					changeMikaChan( _mode );
					changeRipple();
				}, 7000 );
			}
		}
		...
		{
			isClick = true;
			mouse.x = ( ( e.touches[0].pageX - window.pageXOffset ) / window.innerWidth ) * 2 - 1;
			mouse.y = - ( ( e.touches[0].pageY - window.pageYOffset )  / window.innerHeight ) * 2 + 1;
			// e.preventDefault();
			addFieldPower(16);
		}
