...
(function(){
	...
	_gui = new dat.GUI();
	_gui.add( _param, 'auto', _param.augo ).name('AUTO');
	_gui.add( _param, 'time', 0.00, 1.00 ).step(0.01).name('Time').listen();
	...
	*/
	...
	function init()
	{
		...
		_shaderPass.uniforms.h.value = 1 / window.innerWidth * 4.0;
		...
		_shaderPass.uniforms.v.value = 1 / window.innerHeight * 4.0;
		_shaderPass.uniforms.r.value = 0.5;
		...
		_controls.autoRotateSpeed = 0.25;
		...
		_ldr.load(_img, function(_texture){
	...
	function update()
	{
		...
		if( _param.auto )
		{
			_param.time = ( Math.sin( new Date().getTime() * 0.0004 ) + 1.0 ) * 0.5;
		}
		

		if( _mesh ){
			_mesh.material.uniforms.time.value = Math.sin( _param.time * 0.2 * Math.PI * 0.5 ) * 100;
		}
