...
window.onload = function(){
	...
	var light0 = new THREE.AmbientLight( 0x181818 );
	scene.add( light0 );

	var light1 = new THREE.DirectionalLight( 0xFFFFFF, 0.6 );
	scene.add( light1 );
	light1.position.set( 1000, 1250, 1500 );
	...
	scene.add( new THREE.Mesh( new THREE.BoxGeometry( 3600, 3600, 3600, 10, 10, 10 ), new THREE.MeshBasicMaterial({wireframe:true,color: 0x181818}) ) );
	...
	function _createMeteo()
	{
		...
		var _material = new THREE.MeshPhongMaterial( { shininess: 20, shading: THREE.FlatShading } );
		var _mesh = new THREE.Mesh( _geometry, _material );
		scene.add( _mesh );
		_meshList.push( _mesh );
		...
		var mat = new CANNON.Material();
		var boxBody = new CANNON.Body({ mass: 5.0, material: mat });
		//boxBody.angularDamping = 0.01;
		boxBody.addShape(boxShape);
