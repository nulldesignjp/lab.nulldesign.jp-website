...
THREE.RGBNoiseShader = {
	...
	fragmentShader: [
		...
		"void main() {",
		"	float _range = ( fBm( vUv + vec2( time ) ) - 0.5 ) * .2;",
		"	float _x = fBm( vec2(sin(time)*0.16) ) * _range;",
		"	float _y = fBm( vec2(cos(time)*0.12) ) * _range;",
		"	float _z = fBm( vec2(tan(time)*0.15) ) * _range;",
