...
window.onload = function(){
	...
	/*
		https://christmasexperiments.com/2018/04/hatsumode/
		https://twitter.com/christmasxp/status/1069918312572829698
	*/
	...
	function init()
	{
		...
		//	snow
			...
			for( var i = 0; i < len; i++ )
			{
				...
				var _material = new THREE.MeshPhongMaterial({
					map: new THREE.TextureLoader().load('shared/img/mark'+i+'.png'),
					transparent: true,
					color: 0xFFFFFF,
					side: THREE.DoubleSide,
					shininess: 100
				});
				var _mesh00 = new THREE.Mesh( _geometry, _material );
				_world.add( _mesh00 );

				var _rad = i / len * Math.PI * 2.0;
				_mesh00.position.x = Math.cos( _rad ) * 4.0;
				_mesh00.position.z = Math.sin( _rad ) * 4.0;
				_mesh00.position.y = - 5.0;

				_mesh00.rotation.y = - _rad - Math.PI * 0.5;;
			}
	...
	function _loadedHuman( object )
	{
		...
		for( var i = 0; i < 32; i++ )
		{
			...
			var _r = ( 1.0 - Math.random() * Math.random() ) * 32.0;
			var _x = Math.cos( _rad ) * _r;
			var _y = Math.random() * Math.PI * 2.0;
			var _z = Math.sin( _rad ) * _r;
			...
			_m.scale.set( 0.75, 0.75, 0.75 );

			var ray = new THREE.Raycaster(
				new THREE.Vector3( _m.position.x, 100, _m.position.z ),
	...
	function _loadedTree( object )
	{
		...
		for( var i = 0; i < 32; i++ )
		{
			var _s = Math.random() * 4.0 + 2.0;
			var _r = Math.random() * 0.6 + 0.025;
			var _mesh0 = object.children[0].clone();
			_mesh0.material.color = new THREE.Color(0.4,0.4,0.4);
			_mesh0.material.shininess = 0;
