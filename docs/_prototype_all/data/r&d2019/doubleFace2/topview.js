...
var HDYTOP = (function(){
	...
	var _scene, _camera, _renderer, _clock, _plane, _focus;
	var _isRunning = true;
	var _renderKey;
	...
	function init()
	{
		...
		_camera = new THREE.PerspectiveCamera(85, _dom.clientWidth / _dom.clientHeight, 0.1, 3000 );
		...
		_focus = new THREE.Vector3( 0, 110, 0 );
	...
	function setup()
	{
		var _tx = new THREE.TextureLoader().load('message01.png', function(){
			_plane.scale.x = _tx.image.width * 0.25;
			_plane.scale.y = _tx.image.height * 0.25;
		});
			...
			_plane.position.x = - 160;
			_plane.position.y = 160;
			_plane.position.z = 80;
		...
		_mesh.position.y = 110;
		_scene.add( _mesh );


		var _geometry = new THREE.PlaneGeometry( 6400, 6400, 64, 32 );
		_geometry.rotateX( - Math.PI * 0.5 );
		for( var i = 0; i < _geometry.vertices.length; i++ )
		{
			_geometry.vertices[i].y = Math.sin( _geometry.vertices[i].x * _geometry.vertices[i].z ) * 30;
		}
		var _material = new THREE.MeshBasicMaterial({
			wireframe: true,
			transparent: true,
	...
	function mouseupdate( event )
	{
		...
		_camera.position.x = (( _mouse.x - _resolution.x * 0.5) / _resolution.x ) * 3200;
		_camera.position.y = ( 1.0 - _mouse.y / _resolution.y ) * 1200;
	}
	...
	function hdytopShader( _texture )
	{
		var MonoShader = {
			...
			fragmentShader: [
				...
				"void main(void)",
				"{",
				"	gl_FragColor=texture2D( tDiffuse, vUv + vec2( fBm(vUv + vec2(time,vUv.y * 237.0 )) * 0.1 - 0.05, 0.0 ) );",
				"}",
