...
var world = (function(){
	...
	world.prototype = {
		init : function( e )
		{
			...
			// this.scene.fog = new THREE.Fog( 0x13232f, 5, 20 );
			...
			this.renderer.setClearColor( 0x13232f );
		...
		getWorldToScreen2DV : function( vertex )
		{
			var _vertex = vertex.clone();
			var vector = new THREE.Vector3();
			var widthHalf = 0.5*this.renderer.context.canvas.width;
			var heightHalf = 0.5*this.renderer.context.canvas.height;

			var project = _vertex.project(this.camera);
			// vector.x = ( vector.x * widthHalf ) + widthHalf;
			// vector.y = - ( vector.y * heightHalf ) + heightHalf;

			vector.x = widthHalf * (+project.x + 1.0);
			vector.y = heightHalf * (-project.y + 1.0);

			// var _dir0 = new THREE.Vector3().subVectors( this.focus, this.camera.position );
			// var _dir1 = new THREE.Vector3().subVectors( vector, this.camera.position );
			// var _d = _dir0.dot( _dir1 );
			// if( _d <= 0 ){
			// 	vector.x = -9999;
			// 	vector.y = -9999;
