	...
	*/
	...
	function init()
	{
		...
		focus = new THREE.Vector3();
			...
			// camera = new THREE.OrthographicCamera( width / - 2, width / 2, height / 2, height / - 2, 0.1, 32000 );
		...
		_ldr.load('_img01.jpg', function(_texture){
			...
			var imgIndex = new Float32Array( PARTICLES * 1 );
			...
			for( var i = 0; i < len; i++ )
			{
				...
				delay[ i ] = ( ( i % _width ) + Math.floor( ( PARTICLES - i) / _width ) ) / len * 10000.0;

				
				imgIndex[i] = Math.floor( Math.random() * 12 );
			...
			_geometry.addAttribute( 'imgIndex', new THREE.BufferAttribute( imgIndex, 1 ) );
			...
			var yMaterial = new THREE.ShaderMaterial({
				uniforms: {
					...
					opacity: {	value: 1.0	},
					images: { value: [
						new THREE.TextureLoader().load('images/_thumb_00001.jpg'),
						new THREE.TextureLoader().load('images/_thumb_00002.jpg'),
						new THREE.TextureLoader().load('images/_thumb_00003.jpg'),
						new THREE.TextureLoader().load('images/_thumb_00004.jpg'),
						new THREE.TextureLoader().load('images/_thumb_00005.jpg'),
						new THREE.TextureLoader().load('images/_thumb_00006.jpg'),
						new THREE.TextureLoader().load('images/_thumb_00007.jpg'),
						new THREE.TextureLoader().load('images/_thumb_00008.jpg'),
						new THREE.TextureLoader().load('images/_thumb_00009.jpg'),
						new THREE.TextureLoader().load('images/_thumb_00010.jpg'),
						new THREE.TextureLoader().load('images/_thumb_00011.jpg'),
						new THREE.TextureLoader().load('images/_thumb_00012.jpg')
					]}
				},
	...
	function update()
	{
		...
		_time += _clock.getDelta() * 500;

		// mouse.x = window.innerWidth - _time;
