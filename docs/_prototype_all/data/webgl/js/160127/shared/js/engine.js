...
var trektrack = (function(){
	...
	function TrekTrack()
	{
		...
		this.data = [
			...
			{	name: 'ベテルギウス',	english: 'Betelgeuse',	radius: 821300000 * 2,	weight: 0,	deg0: 0,	radius2: 821300000 + 30.06992276 * this.au * 3,	kouten: 164.79,	jiten: 0.6712,	mesh:null,	dom: null,	value:null	}
		];
		...
		setInterval( function(){
			var _no = Math.floor( Math.random() * _t.data.length );
			var _pos = _t.data[_no].mesh;
			_t.target.copy( _pos.position );
		}, 10000 );
		...
		{
			var _d = Math.random() * ( 226000 - 92000 ) + 92000;
			_d *= this.starScale;
			var _rad = Math.random() * Math.PI * 2.0;
			var _x = Math.cos( _rad ) * _d;
			var _y = ( Math.random() - 0.5 ) * 0;
		...
		var _wa = new THREE.PointCloud( _geometry, _material );
	...
	TrekTrack.prototype = 
	{
		loop : function()
		{
			...
			this.world.focus.x += ( this.target.x - this.world.focus.x ) * 0.1;
			this.world.focus.y += ( this.target.y - this.world.focus.y ) * 0.1;
			this.world.focus.z += ( this.target.z - this.world.focus.z ) * 0.1;

			var _accell = 1000;
			var _slow = 1.1200;
			this.vector.x = ( this.vector.x + ( this.target.x - this.world.camera.position.x ) / _accell ) / _slow;
			this.vector.y = ( this.vector.y + ( this.target.y - this.world.camera.position.y ) / _accell ) / _slow;
			this.vector.z = ( this.vector.z + ( this.target.z - this.world.camera.position.z ) / _accell ) / _slow;

			this.world.camera.position.x += this.vector.x;
			this.world.camera.position.y += this.vector.y;
			this.world.camera.position.z += this.vector.z;

			this.world.camera.position.z = 4000;
