...
module hakuhaku
{
	...
	export class main
	{
		...
		constructor( _dom )
		{
			...
			this.scene.fog = new THREE.Fog( 0xFFFFFF, 500, 1800 );
			...
			var _cg = new THREE.BoxGeometry( 3000, 3000, 3000, 1,1,1);
			var _cm = new THREE.MeshBasicMaterial({color:0xFFFFFF,side:THREE.BackSide});
			var _c = new THREE.Mesh( _cg, _cm );
			this.scene.add( _c );
		...
		public engine()
		{
			var len:number = this.ground.geometry.vertices.length;
			for (var i = 0; i < len; i++)
			{
				...
				if( this.count == 6 )
				{
					...
					//_zoom *= 0.5;
					//this.simplexNoise = new SimplexNoise();
					var len:number = _this.ground.geometry.vertices.length;
					for ( var i = 0; i < len; i++ )
					{
						var _value = this.simplexNoise.noise( ( _this.ground.geometry.vertices[i].x + _this.offsetX ) * _this.zoom, ( _this.ground.geometry.vertices[i].y + _this.offsetY ) * _this.zoom );
						_value *= _this.size;
						_this.ground.geometry.vertices[i].z = _value;
					}

					this.radian = ( 0 - this.radian ) * 0.02;
					this.speed *= 0.98;

					this.offsetX += Math.cos( this.radian ) * this.speed;
					this.offsetY += Math.sin( this.radian ) * this.speed;
			...
			this.camera.position.set( Math.sin( Date.now() * 0.0001 ) * 1000, 0, 600 );
		}
		...
		private stepTimer()
		{
			...
			setInterval(function(){
				//_zoom *= 0.5;
				//_this.simplexNoise = new SimplexNoise();
				var len:number = _this.ground.geometry.vertices.length;
				for ( var i = 0; i < len; i++ )
				{
					var _value = _this.simplexNoise.noise( ( _this.ground.geometry.vertices[i].x + _this.offsetX ) * _this.zoom, ( _this.ground.geometry.vertices[i].y + _this.offsetY ) * _this.zoom );
					_value *= _this.size;
					_this.ground.geometry.vertices[i].z += _value;
				}
				...
				if( _this.count %7 == 0 )
				{
					...
					_this.radian = Math.random() * Math.PI * 2 - Math.PI;
					_this.speed = Math.random() * 60 + 60;
					_this.size = Math.random() * 80 + 40;
					_this.zoom = Math.random() * 0.000500 + 0.00100;
			},1000 );
		}

		private createLines( n:number )
		{
			var _blocklist = [
				-1,
				0,
			...
			var _s:number = ( n * 2 ) * Math.floor( ( ( n / 2 ) - 3 ) * 0.5 ) + n - 12 * 0.5 - 2;
			for ( var i = 0; i < _blocklist.length; i++ )
			{
				this.ground.geometry.faces[ _s + _blocklist[i] ].color = new THREE.Color( .20,.20,.20 );
			}
			this.ground.geometry.normalsNeedUpdate = true;
