...
(function(){
	...
	uniforms.time.value = Date.now() % 100;
	...
	function render()
	{
		...
		// {
		// 	len --;
		// 	var _p = particle.geometry.vertices[len];
		// 	_p.x += rnd() * 10;
		// 	_p.y += rnd() * 10;
		// 	_p.z += rnd() * 10;
		// }
	...
	function createParticle()
	{
		...
		for( var i = 0; i < 1000000; i++ )
		{
			...
			//_rad0 = Math.PI * 2.0 * i / 100000;
			//_rad1 = Math.PI * 2.0 * i / 1000000;


			var _pow = ( Math.random()*.08+.96 )*200;
			...
			var _position = new THREE.Vector3( _x, _y, _z );

			_geometry.vertices[i] = _position;

			//	attr
			attributes.size.value[i] = 1.0 * Math.random() * 2.0;
			attributes.customColor.value[i] = new THREE.Color( 0.2,0.5,0.92 );

			attributes.limit.value[i] = Math.random() * 5.0 + 3.0;
			attributes.life.value[i] = Math.random() * attributes.limit.value[i];

			attributes.rad0.value[i] = _rad0;
			attributes.rad1.value[i] = _rad1;
		}
