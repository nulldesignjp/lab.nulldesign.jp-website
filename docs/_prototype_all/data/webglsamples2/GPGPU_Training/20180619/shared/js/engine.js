...
window.onload = function(){
	...
	var _sBox = new THREE.Mesh( _geometry, _material );
	_world.add( _sBox );
	...
	*/
	function loop( _stepTime ){
		...
		//	update....
		_sBox.material.uniforms.texturePosition.value = gpuCompute.getCurrentRenderTarget( positionVariable ).texture;
		_sBox.material.uniforms.textureVelocity.value = gpuCompute.getCurrentRenderTarget( velocityVariable ).texture;
	}
	...
	function generateGeometry(){
		var _box = new THREE.BoxBufferGeometry( 3, 3, 3 );
		console.log( _box )
		...
		for( var j = 0; j < WIDTH; j++ )
		{
			for( i = 0; i < WIDTH; i++ )
			{
				...
				//`_box.attributes.position.count * _box.attributes.position.itemSize
			}
		}
