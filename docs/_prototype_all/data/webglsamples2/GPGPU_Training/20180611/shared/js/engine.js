...
window.onload = function(){
	...
	//	shadow
	var _pGeometry = new THREE.PlaneGeometry( 200, 200, 1, 1 );
	_pGeometry.rotateX( - Math.PI * 0.5 );
	var _pMaterial = new THREE.MeshBasicMaterial({
		map: new THREE.TextureLoader().load('shadow2.png'),
		transparent: true,
		opacity: 0.4,
		//blending: THREE.AdditiveBlending
	});
	var _floor = new THREE.Mesh( _pGeometry, _pMaterial );
	_world.add( _floor );
...
ape1_lowpoly.FBX
ape2_middlepoly.FBX
ape3_jojo_highpoly.FBX
	*/
	loadFBX('untitled.FBX', function(){
	// loadFBX('ape3_jojo_highpoly.FBX', function(){
	// loadFBX('ape3_jojo_highpoly.FBX', function(){
		_FBXgeometry = new THREE.Geometry().fromBufferGeometry( _mesh.children[0].geometry );
		console.log(_FBXgeometry)
		_points = _createparticle();
		_world.add( _points );
		_past = new Date().getTime() * 0.001;
		loop(0);
		console.log('static.');
	...
	function loadFBX( e, _callBack ){
		...
		loader.load( e, function ( object ) {
			...
			_world.focus.set( _c.x, - _c.y * 2.0, _c.z );
			_world.camera.position.y = - _c.y * 2.0;

			_world.camera.position.x *= 2.0;
			_world.camera.position.z *= 2.0;
	...
	function loop( _stepTime ){
		...
		var _current = new Date().getTime() * 0.001;
		var _duration = _current - _past;
		_past = _current;
		var _fps = ~~( 1.0 / _duration );
		$('h2').text('FPS: ' + _fps );
	...
	function _createparticle(){
		...
		for( var i = 0; i < len; i ++ ){
			...
			var _baceScale = Math.random() * 1.8 + 0.2;
			for( var j = 0; j < _len; j++ ){
				_vertices[ _idx++ ] = _pos[j] * _baceScale;
				_normal[ _idy++ ] = _nor[j];
			}
	...
	function getRandomPoint(){
		...
		if( _FBXgeometry )
		{
			...
			_p00 = new THREE.Vector3( - _p00.x * 100.0, _p00.y * 100.0, - _p00.z * 100.0 );
		}
