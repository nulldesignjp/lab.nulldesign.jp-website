...
THREE.BloomShader = {
	...
	fragmentShader: [
        ...
        //  https://github.com/maxillacult/ofxPostGlitch/blob/master/Shaders/glow.frag#L32-L35
        "   float weights = 0.0075;",
        "   for (int i = 0;i < SAMPLE_COUNT*SAMPLE_COUNT;i++){",
        "       bloomColor = bloomColor + max(vec4(0.0), texture2D(tDiffuse,vec2(uv.x+float(mod(float(i),float(SAMPLE_COUNT))) / resolution.x,uv.y+float(i/SAMPLE_COUNT) / resolution.y)) )*weights;",
        "       bloomColor = bloomColor + max(vec4(0.0), texture2D(tDiffuse,vec2(uv.x-float(mod(float(i),float(SAMPLE_COUNT))) / resolution.x,uv.y+float(i/SAMPLE_COUNT) / resolution.y)) )*weights;",
        "       bloomColor = bloomColor + max(vec4(0.0), texture2D(tDiffuse,vec2(uv.x+float(mod(float(i),float(SAMPLE_COUNT))) / resolution.x,uv.y-float(i/SAMPLE_COUNT) / resolution.y)) )*weights;",
        "       bloomColor = bloomColor + max(vec4(0.0), texture2D(tDiffuse,vec2(uv.x-float(mod(float(i),float(SAMPLE_COUNT))) / resolution.x,uv.y-float(i/SAMPLE_COUNT) / resolution.y)) )*weights;",
        "   }",
        "   bloomColor.a = 1.0;",
        //"   bloomColor.rgb *= vec3( 0.1, 0.6, 0.9 );",
		"	gl_FragColor.rgb += bloomColor.rgb;",
		"}"
